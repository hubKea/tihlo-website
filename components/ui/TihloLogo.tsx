...
export default function TihloLogo({
  size = 28,
  light = false,
  className = '',
}: TihloLogoProps) {
  // Base disc + iris-pupil tones flip between light and dark backgrounds.
  const disc = light ? '#FFFFFF' : '#1F2023';
  const sclera = light ? '#1F2023' : '#FFFFFF';
  const pupilFill = light ? '#1F2023' : '#F8F8F8';
  const pupilDot = light ? '#F8F8F8' : '#1F2023';
  const dotInner = light ? '#1F2023' : '#F8F8F8';
  const detail = light ? 'rgba(255,255,255,0.55)' : 'rgba(17,17,17,0.55)';
  ...
  return (
    ...
    >
      ...
      <g mask="url(#tihlo-iris-mask)">
        ...
        {/* Red sighting wedge */}
        <path
          d="M42 42V27C44.5452 27 47.0456 27.6932 49.2498 29.0097C51.454 30.3262 53.2844 32.2198 54.557 34.5L42 42Z"
          fill="#C0272D"
          opacity="0.18"
        />
        <path
          d="M42 42V30.6C43.934 30.6 45.8338 31.1275 47.5085 32.128C49.1833 33.1285 50.574 34.5674 51.541 36.3L42 42Z"
          fill="#C0272D"
          opacity="0.32"
        />
        <path
          d="M42 42V34.8C43.2225 34.8 44.4236 35.1315 45.4825 35.7635C46.5414 36.3955 47.4208 37.3048 48.032 38.4L42 42Z"
          fill="#C0272D"
          opacity="0.45"
        />
        <line
